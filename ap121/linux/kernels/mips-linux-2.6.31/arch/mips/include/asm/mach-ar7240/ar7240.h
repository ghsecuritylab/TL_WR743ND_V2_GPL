...
#else
#ifdef CONFIG_MACH_HORNET
/* this branch is added by zcf, because the irq we need is bigger than 16, so I changed this value, 20110525 */
#define AR7240_GPIO_IRQ_COUNT		30
#else
 ...
 */
#define AR7240_GPIO_IRQn(_gpio)             AR7240_GPIO_IRQ_BASE+(_gpio)
/* the following is not only redundant but also bug-triggering (I invented this word :), by zcf, 20110525  */
//#define AR7240_GPIO_IRQ_COUNT                 16
...
/* Initially, macro ATH_BOOTSTRAP_REG is defined as (AR7240_RESET_BASE + 0xb0). 
*Obviously this is wrong, so we changed it. commented by zcf, 20110503  
*/
#define ATH_BOOTSTRAP_REG		(AR7240_RESET_BASE + 0xac)
