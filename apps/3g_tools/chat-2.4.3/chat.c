...
void set_tty_parameters()
{
    ...
    {
		msgf("%s %d Can't set terminal parameters.",__FUNCTION__,__LINE__);
...
int status;
{
    ...
    msgf("terminate: status == %d \n", status);		/* added by tf, 110607, for debug */
    if (terminating)
	exit(status);
    terminating = 1;
    echo_stderr(-1);
...
int c;
{
    ...
    status = write(1, &ch, 1);
	//msgf("%s %d status = %d, 1 means OK\r",__FUNCTION__,__LINE__,status);
    switch (status) {
    case 1:
	return (0);
...
register char *string;
{
	...
	/* added by ZQQ, 10.11.02 k3760返回的命令带有一些额外的符号: ^->o */
