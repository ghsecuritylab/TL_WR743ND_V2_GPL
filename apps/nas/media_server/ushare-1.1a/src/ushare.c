...
main (int argc, char **argv)
{
	...
	if(1 == isNormal)
	{
		...
		/* wait for usb device was scanned. */
		{
			...
			for (i=0; i<20; i++)
			{
				if (stat("/tmp/usbdisk/volume1",&st) < 0)
				{
					sleep(1);
				}
				else
	...
	else /* for web debug */
	{
		...
		sprintf(gUshareInitInfo.folderList.folderListInfo[0].contentInfo.fullPath, "%s", "/tmp/usbdisk/volume1/mediaRc");
	}
