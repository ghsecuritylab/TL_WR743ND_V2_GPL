							 ...
							 int count, struct upnp_entry_t *entry, char *filter)
{
	...
	for (; *childs; childs++)
	{
		...
		/* only fetch the requested count number or all entries if count = 0 */
		{
			...
			char *protocol = 
#ifdef HAVE_DLNA
				/* when user agent is xbox, do not use dlna profile. by HouXB, 25Oct10 */
				/*(*childs)->dlna_profile?*/
...
#if 0
#ifdef HAVE_DLNA
				(*childs)->dlna_profile ?didl_add_item (out, (*childs)->id,
...
cds_search (struct action_event_t *event)
{
	...
	if ((!strcmp(search_criteria,  "(upnp:class = \"" UPNP_VIDEO "\")")) ||
		(!strcmp(search_criteria,  "(upnp:class = \"" UPNP_AUDIO"\")")) ||
		(!strcmp(search_criteria,  "(upnp:class = \"" UPNP_PHOTO"\")")) )
